...
val LocalHapticFeedbackManager = staticCompositionLocalOf<HapticFeedbackManager> {
    DisabledHapticFeedbackManager
}

@Composable
fun WithHapticFeedbackManager(
    enabled: Boolean = true,
    content: @Composable () -> Unit
) {
    val hapticFeedback = LocalHapticFeedback.current
    val context = LocalContext.current
    val manager = remember(hapticFeedback, context) {
        RealHapticFeedbackManager(context, hapticFeedback).apply {
            setEnabled(enabled)
        }
    }
    CompositionLocalProvider(LocalHapticFeedbackManager provides manager) {
        content()
    }
}
